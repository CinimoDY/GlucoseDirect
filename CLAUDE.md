...
- **Day/night alarm profiles** — Two threshold + volume profiles switched on a fixed user-configured time window (default 22:00→07:00, configurable in Settings → Alarms). `state.alarmHigh / alarmLow / alarmVolume` on `DirectState` are `{ get }`-only **computed accessors** that resolve via `state.activeAlarmProfile`; the actual storage lives in six per-profile + four schedule properties (`dayAlarmHigh`, `nightAlarmHigh`, `nightStartHour`, etc.). Day setters dual-write to legacy `alarmHigh / alarmLow / alarmVolume` UserDefaults keys for safe rollback to a pre-feature build; night setters single-write. Migration in `AppState.init` is once-per-install (gate: `hasMigratedAlarmProfiles`) and copies legacy values into both profiles or seeds defaults (180/80/0.2). Treatment cycle, snooze, predictive low, and critical-low breakthrough (`isCriticalLow` helper in `Library/Content/AlarmProfile.swift`) all read the active profile transparently — no cycle pinning. Sensor end-of-life warnings use `max(dayAlarmVolume, nightAlarmVolume)` so a silenced night never suppresses an overnight failure. Cross-target sync: `AppGroupSharing.writeAlarmProfileData(state:)` mirrors schedule + per-profile thresholds to `UserDefaults.shared` on `.startup`, `.addSensorGlucose`, and each non-volume per-profile setter (volume excluded — slider drag would burn the ActivityKit push budget without user-visible benefit). Live Activity ContentState carries 8 optional schedule + per-profile fields plus the legacy day-anchored `alarmLow / alarmHigh` for pre-upgrade in-flight activities. **Render-time resolution gotcha:** Home Screen widget pre-resolves entries at `getTimeline` build time, so a single-entry timeline can show stale thresholds for up to the reload window past a boundary; `GlucoseUpdateProvider` emits a second timeline entry at the next boundary via `nextAlarmProfileBoundary(...)`. Live Activity is naturally correct via `effectiveAlarmThresholds(at:)` called from the view body. See `docs/solutions/best-practices/widgetkit-timeline-time-of-day-boundary-entries-20260503.md`.
